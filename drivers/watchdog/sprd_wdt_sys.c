...
#define pr_info(fmt, ...) \
	printk(KERN_ERR "[SPRD_WD] "fmt, ##__VA_ARGS__)
#define pr_debug(fmt, ...) \
	printk(KERN_ERR "[SPRD_WD] "fmt, ##__VA_ARGS__)
#define pr_err(fmt, ...) \
	printk(KERN_ERR "[SPRD_WD] "fmt, ##__VA_ARGS__)
#define pr_warning(fmt, ...) \
	printk(KERN_WARNING "[SPRD_WD] "fmt, ##__VA_ARGS__)
#else
#define pr_debug(fmt, ...)
#define pr_info(fmt, ...)
#define pr_err(fmt, ...) \
...
#if 1 /* HTC_CSP_START */
/* re-config the tick and timeout value */
static int feed_period = 10; /* (secs) Default is 3 sec */
static int ca7_irq_margin = 80; /* (secs) Default is 9 sec */
static int ca7_margin = 85; /* (secs) Default is 10 sec */
static int ap_margin = 100; /* (secs) Default is 12 sec */
#if KERNEL_ONLY_CHIP_DOG
static int chip_margin = 120; /* (secs) Default is 8 sec */
#else
static int chip_margin = 120; /* (secs) Default is 15 sec */
#endif
...
#else
extern int fiq_printk_lock(int stage);
static void ca7_wdg_fiq(struct fiq_glue_handler *h, void *regs, void *svc_sp)
{
	...
	mdelay(200); /* waiting dump finished */
...
{
#if 1 /* HTC_CSP_START */
	int adie_ret = 0, ap_ret = 0, ca7_ret = 0;
	int adie_val = 0, ap_val = 0, ca7_val = 0;

	pr_debug("%s + : wdt_enabled=%ld\n", __FUNCTION__, wdt_enabled);
#else
	pr_debug("%s\n", __FUNCTION__);
#endif /* HTC_CSP_END */
	...
	ENABLE_ALL_WDG();
#else
	adie_ret = sci_adi_set(WDG_CTRL, WDG_CNT_EN_BIT | WDG_RST_EN_BIT);
	ap_ret = sci_glb_set(AP_WDG_CTRL, WDG_CNT_EN_BIT | WDG_RST_EN_BIT);
	ca7_ret = sci_glb_set(CA7_WDG_CTRL, WDG_CNT_EN_BIT | WDG_RST_EN_BIT | WDG_INT_EN_BIT);
	pr_debug("%s : set adie_ret=%d, ap_ret=%d, ca7_ret=%d \n", __FUNCTION__, adie_ret, ap_ret, ca7_ret);

	adie_val = sci_adi_read(WDG_CTRL);
	ap_val = sci_glb_read(AP_WDG_CTRL, -1UL);
	ca7_val = sci_glb_read(CA7_WDG_CTRL, -1UL);
	pr_debug("%s : read adie_val=%d, ap_val=%d, ca7_val=%d \n", __FUNCTION__, adie_val, ap_val, ca7_val);

	pr_debug("%s -\n", __FUNCTION__);

	/* flush the first stage resume log from cache to printk buffer in RAM */
	flush_cache_all();
#endif /* HTC_CSP_END */
	return;
}
...
static int sprd_wdt_suspend(void)
{
	...
	DISABLE_ALL_WDG();
#else
	adie_ret = sci_adi_clr(WDG_CTRL, WDG_CNT_EN_BIT | WDG_RST_EN_BIT);
	ap_ret = sci_glb_clr(AP_WDG_CTRL, WDG_CNT_EN_BIT | WDG_RST_EN_BIT);
	ca7_ret = sci_glb_clr(CA7_WDG_CTRL, WDG_CNT_EN_BIT | WDG_RST_EN_BIT | WDG_INT_EN_BIT);
	pr_debug("%s : set adie_ret=%d, ap_ret=%d, ca7_ret=%d \n", __FUNCTION__, adie_ret, ap_ret, ca7_ret);
...
static int watchdog_feeder(void *data)
{
	do {
		if (kthread_should_stop())
			break;
		pr_debug("%s, ca7_irq_margin=%d, ca7_margin=%d, ap_margain=%d, chip_margin=%d, feed_period=%d, wdt_enabled=%ld\n",
			__FUNCTION__, ca7_irq_margin, ca7_margin, ap_margin, chip_margin, feed_period, wdt_enabled);
