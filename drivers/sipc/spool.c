		...
		char __user *buf, size_t count, loff_t *ppos)
{
    ...
    if((ret = sblock_receive(sblock->dst, sblock->channel, &blk, timeout)) < 0){
        LOGD("spool_read: failed to receive block!\n");
        return ret;
    }
    ...
    if(copy_to_user(buf, blk.addr, rdsize)){
        LOGD("spool_read: failed to copy to user!\n");
        ret = -EFAULT;
    }else{
        ret = rdsize;
    }
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    do{
        if((ret = sblock_get(sblock->dst, sblock->channel, &blk, timeout)) < 0){
            LOGW("spool_write: failed to get block!\n");
            return ret;
        }

        wrsize = (blk.length > len ? len : blk.length);
	LOGD("spool_write: blk_len %d, count %d, wsize %d\n", blk.length, len, wrsize);
        if(copy_from_user(blk.addr, buf + pos, wrsize)){
            LOGW("spool_write: failed to copy from user!\n");
            ret = -EFAULT;
        }else{
            blk.length = wrsize;
            len -= wrsize;
        ...
        if(sblock_send(sblock->dst, sblock->channel, &blk)){
            LOGD("spool_write: failed to send block!");
        }
	LOGD("spool_write len= %u, ret= %d\n", len, ret);
    }while(len > 0 && ret == 0);
...
static int __devinit spool_probe(struct platform_device *pdev)
{
	...
	if (rval != 0) {
		LOGE("Failed to create sblock: %d\n", rval);
		return rval;
	}
	...
	if (spool == NULL) {
		...
		LOGE("Failed to allocate spool_device\n");
		return -ENOMEM;
	}
	...
	if (rval != 0) {
		sblock_destroy(init->dst, init->channel);
		kfree(spool);
		LOGE("Failed to alloc spool chrdev\n");
		return rval;
	}
	/*TODO block device ? */
