...
struct projector_dev {
	...
	unsigned int profiling_fps;
	struct timer_list profiling_timer;
};
...
void android_switch_htc_mode(void)
{
	...
	/* no USB_FUNCTION_SERIAL. USB_FUNCTION_AUTOBOT creates three tty ports in STE code base */
		...
		unsigned intf, unsigned alt)
{
	...
	if (ret) {
		printk("===project set alt fail 1\n");
		return ret;
	}
	ret = usb_ep_enable(dev->ep_out);
	if (ret) {
		printk("===project set alt fail 2\n");
	...
	dev->online = 1;
	dev->profiling_fps= 0;
	mod_timer(&dev->profiling_timer,jiffies + 10 *HZ);
	projector_queue_out(dev);
...
static void projector_show_current_fps(unsigned long dev_pointer)
{
	struct projector_dev *dev = (struct projector_dev *) dev_pointer;
	printk("[USB]projector fps %d\n",dev->profiling_fps / 10);
	dev->profiling_fps = 0;
	if (dev->online == 0)
		return;
	mod_timer(&dev->profiling_timer, jiffies + 10 * HZ);
...
static int projector_setup(struct htcmode_protocol *config)
{
	...
	init_timer(&dev->profiling_timer);
	dev->profiling_timer.function = projector_show_current_fps;
	dev->profiling_timer.data = (uint32_t)dev;
