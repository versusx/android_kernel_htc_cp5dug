...
//#define CONFIG_USB_FUNCTION_PERFLOCK 0
//#endif
				...
				struct usb_endpoint_descriptor *intr_desc)
{
	...
	ep = usb_ep_autoconfig_bulk_doublebuffer(cdev->gadget, in_desc);
	if (!ep) {
	...
	ep = usb_ep_autoconfig_bulk_doublebuffer(cdev->gadget, out_desc);
	if (!ep) {
