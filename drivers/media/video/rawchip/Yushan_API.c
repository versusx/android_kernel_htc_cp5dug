...
bool_t	Yushan_Init_LDO(bool_t	bUseExternalLDO)
{
	...
	if (bSpiReadData == 1) {
		...
		if (!fStatus) {
			DEBUGLOG("[CAM] %s:LDO Interrupt Failed\n", __func__);
...
/*  Compute the lower limit of DxO clock:
	...
	while (bCount >= 0) {
		...
		if ((fpDxo_Clk >= fpDxoClockLow) && (fpDxo_Clk <= DXO_CLK_LIMIT)) {
			...
			if (sInitStruct->bDxoSettingCmdPerFrame == 1)
				udwFrameBlank += (135000/udwFullLine)+1; /* For 1 SettingsPerFrame. */
			if (sInitStruct->bDxoSettingCmdPerFrame > 1)
				udwFrameBlank += (220000/udwFullLine)+1; /* For more than 1 SettingsPerFrame. */
	...
	/* Writing DXO, SYS and IDP Dividers to their respective registers */
	/* Converting fixed point to non fixed point  */
	/* Considering decimal points too for dividers */
	bSpiData = (uint8_t)(fpDxoClk_Div>>16); 			/* Integer part of divider */
	bSpiData = bSpiData<<1;
	...
	bSpiData = (uint8_t)(fpIdpClk_div>>16); 			/* Integer part of divider */
	bSpiData = bSpiData<<1;
	...
	bSpiData = (uint8_t)(fpSysClk_Div>>16); 			/* Integer part of divider */
	bSpiData = bSpiData<<1;
...
bool_t	Yushan_Init_Dxo(Yushan_Init_Dxo_Struct_t *sDxoStruct, bool_t fBypassDxoUpload)
{
	...
	if (!fBypassDxoUpload) {
		fStatus &= SPI_Write((uint16_t)(sDxoStruct->uwBaseAddrDopMicroCode[0]+udwDxoBaseAddress), sDxoStruct->uwDxoDopRamImageSize[0], sDxoStruct->pDxoDopRamImage[0]);
		fStatus &= SPI_Write((uint16_t)(sDxoStruct->uwBaseAddrDopMicroCode[1]+udwDxoBaseAddress), sDxoStruct->uwDxoDopRamImageSize[1], sDxoStruct->pDxoDopRamImage[1]);
	}
	...
	if (!fBypassDxoUpload) {
		fStatus &= SPI_Write((uint16_t)(sDxoStruct->uwBaseAddrDppMicroCode[0] + udwDxoBaseAddress), sDxoStruct->uwDxoDppRamImageSize[0], sDxoStruct->pDxoDppRamImage[0]);

		/*  Setting base address to the DPP calibration data base address. */
		udwSpiBaseIndex = DXO_DPP_BASE_ADDR + sDxoStruct->uwBaseAddrDppMicroCode[1]; /*  0x18000; */
		SPI_Write(YUSHAN_HOST_IF_SPI_BASE_ADDRESS, 4, (uint8_t *)(&udwSpiBaseIndex));
		...
		fStatus &= SPI_Write((uint16_t)(udwDxoBaseAddress), sDxoStruct->uwDxoDppRamImageSize[1], sDxoStruct->pDxoDppRamImage[1]);
	}
	...
	/*  Boot Sequence */
	/*  As per the scenerio files, first program the start address of the code */
	/*  --------------------( Actual Addr - SpiBaseIndex + 0x8000) */
	...
	/*  Reinstating SPIBaseIndex to 0x8000 */
	...
	if (!fBypassDxoUpload) {
		fStatus &= SPI_Write((uint16_t)(sDxoStruct->uwBaseAddrPdpMicroCode[0] + DXO_PDP_BASE_ADDR), sDxoStruct->uwDxoPdpRamImageSize[0], sDxoStruct->pDxoPdpRamImage[0]);
		fStatus &= SPI_Write((uint16_t)(sDxoStruct->uwBaseAddrPdpMicroCode[1] + DXO_PDP_BASE_ADDR), sDxoStruct->uwDxoPdpRamImageSize[1], sDxoStruct->pDxoPdpRamImage[1]);
	}
	...
	fStatus &= Yushan_WaitForInterruptEvent(EVENT_DOP7_BOOT, TIME_10MS);
	if (!fStatus) {
	...
	udwHWnMicroCodeID &= 0xFFFFFF00; 	/*  Not checking the ucode minor version and calibration ids */
	if ((udwHWnMicroCodeID != DXO_DOP_HW_MICROCODE_ID)) {
		ERRORLOG("[CAM] read MicroCodeID = %x, expect %x", udwHWnMicroCodeID, DXO_DOP_HW_MICROCODE_ID);
	...
	if (udwHWnMicroCodeID != DXO_DPP_HW_MICROCODE_ID) {
		ERRORLOG("[CAM] read MicroCodeID = %x, expect %x", udwHWnMicroCodeID, DXO_DPP_HW_MICROCODE_ID);
	...
	if (udwHWnMicroCodeID != DXO_PDP_HW_MICROCODE_ID) {
		ERRORLOG("[CAM] read MicroCodeID = %x, expect %x", udwHWnMicroCodeID, DXO_PDP_HW_MICROCODE_ID);
...
bool_t Yushan_Init(Yushan_Init_Struct_t *sInitStruct)
{
	...
	/*  YUSHAN_CSI2_RX_BYTE2PIXEL_READ_TH */
	/* udwSpiData = 32; */
	udwSpiData = 4; 	/*  For Debug only */
	...
	udwSpiData = 0xc810;
	SPI_Write(YUSHAN_IDP_GEN_ERROR_LINES_EOF_GAP, 3, (unsigned char *)&udwSpiData);
...
bool_t Yushan_Update_Commit(uint8_t  bPdpMode, uint8_t  bDppMode, uint8_t  bDopMode)
{
	...
	/* execution command */
	fStatus = Yushan_WaitForInterruptEvent(EVENT_PDP_EOF_EXECCMD, TIME_50MS);
	if (!fStatus) {
	...
	fStatus = Yushan_WaitForInterruptEvent(EVENT_DOP7_EOF_EXECCMD, TIME_50MS);
	if (!fStatus) {
	...
	fStatus = Yushan_WaitForInterruptEvent(EVENT_DPP_EOF_EXECCMD, TIME_50MS);
	if (!fStatus) {
...
bool_t Yushan_Intr_Enable(uint8_t *pIntrMask)
{
	...
	while (bIntrCount <= (TOTAL_INTERRUPT_COUNT+1)) {
		...
		if (bIntrCountInSet == bTotalEventInASet) {
			...
			/*  Complement of udwSpiData will be udwCompSpiData. */
			...
			/*  Since clks are not driven by Pll, no auto  */
			/*  incremental writes. */
			if (gPllLocked) {
				/* Get number of bytes to write */
				ByteCount = (uint16_t)((bTotalEventInASet+7)>>3);
				/* Write udwSpiData to the Interrupt Enable register */
				if (udwSpiData != 0)
					/* Need to write in register only if there is at least one interrupt to enable */
				...
				if (udwCompSpiData != 0)
					/* Need to write in register only if there is at least one interrupt to disable */
...
/******************************************************************
API Function:			Yushan_ReadIntr_Status
Input:					InterruptList
...
bool_t Yushan_Intr_Status_Clear(uint8_t *bListOfInterrupts)
{
	...
	while (bIntrCount <= (TOTAL_INTERRUPT_COUNT + 1)) {
		...
		/*  location more than the number of interrupts/events. */
		if (bIntrCountInSet == bTotalEventInASet) {
			/* Write only in the register if there is at least one interrupt to clear */
			if (udwSpiData != 0) {
				/* Get number of bytes to write */
				ByteCount = (uint16_t)((bTotalEventInASet+7)>>3);
				/*  InterruptSet offset from the Interrupt base address. */
				uwIntrSetOffset = (bIntrSetID * INTERRUPT_SET_SIZE);
...
bool_t	Yushan_Context_Config_Update(Yushan_New_Context_Config_t	*sYushanNewContextConfig)
{
	...
	/* Read current streaming mode and set-up new accordingly */
	/* If not yet started streming, do not change video mode */
	bCurrentStreamingMode = Yushan_GetCurrentStreamingMode();
	if (bCurrentStreamingMode == YUSHAN_FRAME_FORMAT_STILL_MODE) {
		/* Current streaming mode is STILL, so selected mode will be VF */
		...
		if (bCurrentStreamingMode == YUSHAN_FRAME_FORMAT_VF_MODE) {
			/* Current streaming mode is VF, so selected mode will be STILL */
...
/******************************************************************
API Function:	This function check MIPI TX Clk lane state
				if it is in HS or ULPM mode.
Input:			None
Return:			TRUE if current mode is HS
				FALSE if current mode is ULPM
*******************************************************************/
bool_t Yushan_GetState_Tx_HsClock()
{
	uint8_t		bSpiData = 0;
	bool_t		fCurrentHsState;

	VERBOSELOG("[CAM] %s: Start\n", __func__);

	/* Read Current state */
	SPI_Read(YUSHAN_MIPI_TX_ENABLE, 1, (unsigned char *)&bSpiData);
	fCurrentHsState = (((bSpiData>>1)&0x01) == 1)?TRUE:FALSE;

	VERBOSELOG("[CAM] %s:MIPI TX CLK lane currentmode is %s.\n", __func__, fCurrentHsState?"HS":"ULPM");
	VERBOSELOG("[CAM] %s: End\n", __func__);
	return fCurrentHsState;
}


/******************************************************************
API Function:	This function will set MIPI TX Clk lane to HS or
				ULPM mode depending of argument value.
Input:			EnableHsClk.
				TRUE: Set MIPI TX Clk lane to HS mode
				FALSE:Set MIPI TX Clk lane to ULPM mode
Return:			Success or Failure
*******************************************************************/
bool_t Yushan_Enable_Tx_HsClock(bool_t fEnableHsClk)
{
	bool_t		fStatus; /*  = SUCCESS; */
	uint8_t		bSpiData = 0;
	bool_t		fCurrentHsState;
	...
	if (fCurrentHsState == fEnableHsClk) {
		VERBOSELOG("[CAM] %s:MIPI TX CLK lane already set to %s.\n", __func__, fCurrentHsState?"HS":"ULPM");
		fStatus = SUCCESS;
	} else {
		bSpiData = (bSpiData & ~(0x02)) | (fEnableHsClk<<1);
		fStatus = SPI_Write(YUSHAN_MIPI_TX_ENABLE, 1, &bSpiData);
		VERBOSELOG("[CAM] %s:MIPI TX CLK lane switched to %s.\n", __func__, fCurrentHsState?"HS":"ULPM");
	}
...
Return:
*******************************************************************/
#define YUSHAN_REGISTER_CHECK(reg_addr) udwSpiData = 0; SPI_Read(reg_addr, 4, (uint8_t *)(&udwSpiData)); printk("[CAM] %s: %s: 0x%08x\n", __func__, #reg_addr, udwSpiData);
#define YUSHAN_DOP_REGISTER_CHECK(reg_addr) udwSpiData = 0; SPI_Read(reg_addr + DXO_DOP_BASE_ADDR, 1, (uint8_t *)(&udwSpiData)); printk("[CAM] %s: %s: 0x%02x\n", __func__, #reg_addr, udwSpiData);
#define YUSHAN_PDP_REGISTER_CHECK(reg_addr) udwSpiData = 0; SPI_Read(reg_addr + DXO_PDP_BASE_ADDR, 1, (uint8_t *)(&udwSpiData)); printk("[CAM] %s: %s: 0x%02x\n", __func__, #reg_addr, udwSpiData);
#define YUSHAN_DPP_REGISTER_CHECK(reg_addr) udwSpiData = 0; SPI_Read(reg_addr + DXO_DPP_BASE_ADDR-0x8000, 1, (uint8_t *)(&udwSpiData)); printk("[CAM] %s: %s: 0x%02x\n", __func__, #reg_addr, udwSpiData);
#define YUSHAN_DPP_REGISTER_CHECK2(reg_addr) udwSpiData = 0; SPI_Read(reg_addr + DXO_DPP_BASE_ADDR-0x10000, 1, (uint8_t *)(&udwSpiData)); printk("[CAM] %s: %s: 0x%02x\n", __func__, #reg_addr, udwSpiData);
void Yushan_Status_Snapshot(void)
{
	...
	DEBUGLOG("[CAM] %s: **** MIPI RX CONFIG  CHECK ****\n", __func__);
	YUSHAN_REGISTER_CHECK(YUSHAN_ITM_RX_PHY_STATUS);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_ENABLE);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_UIX4);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_SWAP_PINS);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_INVERT_HS);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_STOP_STATE);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_ULP_STATE);
	YUSHAN_REGISTER_CHECK(YUSHAN_MIPI_RX_CLK_ACTIVE);
	...
	DEBUGLOG("[CAM] %s: **** CSI2 TX INTERFACE  CHECK ****\n", __func__);
