...
#define DCAM_IRQ_LINE_MASK                             0x00077FFFUL  
...
LOCAL uint32_t                    s_path2_done_cnt = 0;
LOCAL uint32_t                    s_path2_done_wait = 0;
LOCAL struct semaphore            s_path2_done_sema = __SEMAPHORE_INITIALIZER(s_path2_done_sema, 0);
...
static uint32_t                *tmp_buf = NULL;
static uint16_t path1_ncnt = 0;
static uint16_t path2_ncnt = 0;
static uint16_t path2_wait_ncnt = 0;
...
void dcam_glb_reg_awr(uint32_t addr, uint32_t val, uint32_t reg_id)
{
	...
	case DCAM_CFG_REG:
		...
		__raw_writel(__raw_readl(addr) & (val), addr);
...
void dcam_glb_reg_mwr(uint32_t addr, uint32_t mask, uint32_t val, uint32_t reg_id)
{
	...
	case DCAM_CFG_REG:
		spin_lock_irqsave(&dcam_glb_reg_cfg_lock, flag);
		{
			...
			tmp &= ~(mask);
			__raw_writel(tmp | ((mask) & (val)), addr);
		}
		spin_unlock_irqrestore(&dcam_glb_reg_cfg_lock, flag);
		break;
	case DCAM_CONTROL_REG:
	              ...
	              enum dcam_cap_sensor_mode sn_mode)
{
	...
	if (if_mode >= DCAM_CAP_IF_MODE_MAX) {
		...
		if (sn_mode >= DCAM_CAP_MODE_MAX) {
			...
			sema_init(&s_dcam_mod.resize_done_sema, 0);
			sema_init(&s_dcam_mod.rotation_done_sema, 0);
			_dcam_link_frm(0); 
	...
	if(0 == g_dcam_irq_en){
		...
		dcam_glb_reg_mwr(DCAM_INT_MASK, DCAM_IRQ_LINE_MASK, DCAM_IRQ_LINE_MASK, DCAM_INIT_MASK_REG);
...
int32_t dcam_module_en(void)
{
	...
	if (atomic_inc_return(&s_dcam_users) == 1) {
...
#if 0
		{
			...
			REG_MWR(SPRD_MMAHB_BASE+0x4, bit_value, bit_value); 
			...
			REG_MWR(SPRD_MMCKG_BASE + 0x2c, 0xf, 0x3);  

			REG_MWR(SPRD_MMCKG_BASE + 0x20, 0xf, 0x3);  
		}
#endif
...
int32_t dcam_module_dis(void)
{
	...
	if (atomic_dec_return(&s_dcam_users) == 0) {
...
#if 0
		{
			...
			REG_MWR(SPRD_MMAHB_BASE+0x8, bit_value, 0); 

			bit_value = BIT_0 | BIT_1| BIT_4;
			REG_MWR(SPRD_MMAHB_BASE, bit_value, 0);  
...
int32_t dcam_start_path(enum dcam_path_index path_index)
{
	...
	if ((DCAM_PATH_IDX_0 & path_index) && s_dcam_mod.dcam_path0.valide) {
		...
		if (cap_en) {
			...
			dcam_glb_reg_mwr(DCAM_CONTROL, BIT_1, 1 << 1, DCAM_CONTROL_REG); 
		} else {
			dcam_glb_reg_owr(DCAM_CFG, BIT_0, DCAM_CFG_REG);             
		}
	}
	...
	if (DCAM_PATH_IDX_ALL != path_index) {
		if ((DCAM_PATH_IDX_0 & path_index)) {
			...
			s_path2_done_cnt = 1;
			s_path2_done_wait = 1;
			rtn = down_timeout(&s_path2_done_sema, msecs_to_jiffies(500));
			if (rtn) {
				s_path2_done_wait = 0;
				printk("DCAM DRV: Wait path2 done: Failed wait s_path2_done_sema \n");
			} else {
				DCAM_TRACE("DCAM DRV: wait s_path2_done_sema done \n");
			}
...
int32_t dcam_stop_cap(void)
{
	...
	dcam_glb_reg_mwr(DCAM_CONTROL, BIT_2, 0, DCAM_CONTROL_REG); 
	
	DCAM_TRACE("DCAM DRV: dcam_stop_cap, s_resize_flag %d \n", atomic_read(&s_resize_flag));
	if (atomic_read(&s_resize_flag)) {
		s_dcam_mod.wait_resize_done = 1;
		rtn = down_timeout(&s_dcam_mod.resize_done_sema, DCAM_PATH_TIMEOUT);
	}
	if (atomic_read(&s_rotation_flag)) {
		s_dcam_mod.wait_rotation_done = 1;
		rtn = down_timeout(&s_dcam_mod.rotation_done_sema, DCAM_PATH_TIMEOUT);
	}
...
void _dcam_wait_sof()
{
	...
	s_sof_cnt = 0;
	s_sof_wait = 1;
	while(!down_trylock(&s_path_stop_sema)){
		printk("DCAM DRV: sof already buffered, clear!!!\n");
	}
	rtn = down_timeout(&s_path_stop_sema, msecs_to_jiffies(200));
	if (rtn) {
		s_sof_wait = 0;
...
int32_t dcam_resume(void)
{
	...
	dcam_glb_reg_owr(DCAM_CONTROL, BIT_2, DCAM_CONTROL_REG); 
	return -rtn;
}
		...
		cap_dec->y_factor > DCAM_CAP_Y_DECI_FAC_MAX ) {
			...
			if (DCAM_CAP_IF_CCIR == cap_desc->interface) {
				...
				if (DCAM_CAP_MODE_RAWRGB == cap_desc->input_format) {
					...
					REG_MWR(CAP_MIPI_IMG_DECI, BIT_1, cap_dec->x_factor << 1);
				} else {
...
#define ION_SIZE_ADDR			0x07C00000
			} else if( (p_addr->yaddr < ION_BASE_ADDR) || (p_addr->yaddr > (ION_BASE_ADDR + ION_SIZE_ADDR)) ||
				   (p_addr->uaddr < ION_BASE_ADDR) || (p_addr->uaddr > (ION_BASE_ADDR + ION_SIZE_ADDR)) ) {
				rtn = DCAM_RTN_PATH_ADDR_ERR;
				DCAM_TRACE("DCAM DRV: Path 2 DCAM_PATH_OUTPUT_ADDR Error, i=%d, lock=%d, y=0x%x, u=0x%x, v=0x%x \n",
					path->output_frame_count, s_path2_frame[path->output_frame_count].lock,p_addr->yaddr, p_addr->uaddr, p_addr->vaddr);
				DCAM_TRACE("DCAM DRV: Ion mem range: %08x - %08x\n", ION_BASE_ADDR, (ION_BASE_ADDR + ION_SIZE_ADDR));
#undef ION_BASE_ADDR
#undef ION_SIZE_ADDR
#endif
			} else {
...
int32_t    dcam_resize_end(void)
{
	if (atomic_read(&s_resize_flag)) {
		...
		if (s_dcam_mod.wait_resize_done) {
			up(&s_dcam_mod.resize_done_sema);
			s_dcam_mod.wait_resize_done = 0;
		}
	}
	return 0;
}
...
int32_t    dcam_rotation_end(void)
{
	...
	if (s_dcam_mod.wait_rotation_done) {
		up(&s_dcam_mod.rotation_done_sema);
		s_dcam_mod.wait_rotation_done = 0;
	}
	return 0;
}
...
LOCAL irqreturn_t dcam_isr_root(int irq, void *dev_id)
{
...
#if 0
			printk("DCAM DRV: ignore path2, int=0x%x, valid=%d, status=%d isr_flag %d\n",
				status, s_dcam_mod.dcam_path2.valide, s_dcam_mod.dcam_path2.status, path2_isr_flag);
			path2_isr_flag ++;
			if(path2_isr_flag < 10){
				status &= ~((1 << PATH2_DONE) | (1 << PATH2_OV));
				irq_line = status;
			}else{
				irq_line &= ~((1 << PATH2_DONE) | (1 << PATH2_OV));
			}
		}
#endif
		return IRQ_NONE;
	...
	if (err_flag && s_dcam_mod.user_func[DCAM_TX_ERR]) {
		...
		if(irq_line & ((1 << PATH2_DONE) | (1 << PATH2_OV))) {
			path2_isr_flag = 0;
		}
		for (i = IRQ_NUMBER - 1; i >= 0; i--) {
			if (irq_line & (1 << (uint32_t)i)) {
				if(NULL != isr_list[i]){
					...
					printk("[CAM]lqyuan isr_list[%d] NULL\n",i);
				}
			}
	...
	dcam_glb_reg_owr(DCAM_INT_CLR, status, DCAM_INIT_CLR_REG);
...
LOCAL void _dcam_path0_set(void)
{
	...
	dcam_glb_reg_mwr(DCAM_ENDIAN_SEL, BIT_18, BIT_18, DCAM_ENDIAN_REG); 
	dcam_glb_reg_mwr(DCAM_ENDIAN_SEL, BIT_19, BIT_19, DCAM_ENDIAN_REG); 

		DCAM_TRACE("testest: path endian: %d \n",path->data_endian.y_endian );
...
LOCAL int32_t _dcam_set_sc_coeff(enum dcam_path_index path_index)
{
	...
	if (NULL == tmp_buf) {
		DCAM_TRACE("DCAM DRV: _dcam_set_sc_coeff alloc mem fail");
		return -DCAM_RTN_PATH_NO_MEM;
	}

	DCAM_TRACE("tmp buffer %p\n",tmp_buf);
	h_coeff = tmp_buf;
...
LOCAL void _dcam_read_ddr()
{
	printk("[CAM]DDR 0x%x \n", REG_RD(SPRD_AONAPB_BASE + 0x0018));
}
...
LOCAL void    _path2_done(void)
{
	...
	if(path2_ncnt++ % 30 == 0){
		printk("DCAM path2 frame %d \n", path2_ncnt);
		
	}

	if (s_path2_done_wait) {
		s_path2_done_cnt--;
		DCAM_TRACE("DCAM: _path2_done: %d \n", s_path2_done_cnt);
		if (0 == s_path2_done_cnt) {
			s_path2_done_wait = 0;
			up(&s_path2_done_sema);
			DCAM_TRACE("DCAM: _path2_done %d, send s_path2_done_sema\n", s_path2_done_cnt);	
		}
	}
	...
	if (s_dcam_mod.dcam_path2.path_update) {
		...
		rtn = _dcam_path_set_next_frm(DCAM_PATH_IDX_2, false);
		if (rtn && path2_wait_ncnt ++ % 30 == 0) {
			DCAM_TRACE("DCAM DRV: path 2 wait for frame unlocked, count is %d \n", path2_wait_ncnt);
			return;
		}
		_dcam_auto_copy(DCAM_PATH_IDX_2);
	}
