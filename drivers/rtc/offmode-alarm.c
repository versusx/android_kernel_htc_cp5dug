...
static unsigned int offalarm[OFFALARM_SIZE];
...
{
	struct timespec rtc_now, ts;
	int next_alarm = 0, rtc_offalarm = 0;
	int i, rv;
	int next_alarm_snooze = 0;

	getnstimeofday(&rtc_now);

	for (i = 0; i < offalarm_size; i++) {
		printk(KERN_INFO "[OffAlarm][%d] %d \n", i, offalarm[i]);
		if (offalarm[i] > rtc_now.tv_sec) {
			if (next_alarm == 0)
				next_alarm = offalarm[i];
			else if ( offalarm[i] < next_alarm)
				next_alarm = offalarm[i];
		}
	}
	...
	for (i = 0; i < offalarm_snooze_size; i++) {
                if (offalarm_snooze[i] > rtc_now.tv_sec) {
                        if (next_alarm_snooze == 0)
                                next_alarm_snooze = offalarm_snooze[i];
                        else if (offalarm_snooze[i] < next_alarm_snooze)
                                next_alarm_snooze = offalarm_snooze[i];
		}
	}

	printk(KERN_INFO "[OffAlarm] alarm(%u), snooze(%u)\n", next_alarm, next_alarm_snooze);

	//HTC_KER_MARK: if next_alarm is 0 and next_alarm_snooze !=0, rtc_offalarm value should be next_alarm_snooze.
	//if(next_alarm_snooze <= next_alarm && next_alarm_snooze != 0)

	if((next_alarm == 0 || next_alarm_snooze <= next_alarm) && next_alarm_snooze != 0)
		rtc_offalarm = next_alarm_snooze;
	else
		rtc_offalarm = next_alarm;

	rtc_offalarm -= 30;

	ts.tv_sec = rtc_offalarm;
	ts.tv_nsec = 0;

	rv = alarm_set_rtc_alarm(ts);
...
early_param("enable_offalarm", enable_offalarm_setup);
