...
static DEVICE_ATTR(function_switch, S_IRUGO | S_IWUSR, switch_show,
		switch_store);
...
int tps61310_flashlight_control(int mode)
{
	...
	if (this_tps61310->led_count == 1) {
		...
		case FL_MODE_OFF:
			flashlight_turn_off();
		break;
		case FL_MODE_CAMERA_EFFECT_FLASH://HTC_CAM_MOD, Chuck Disable timer because it's implemted by ISP_FW.
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x9E);

		break;
		case FL_MODE_CAM_FLASH_LEVEL1://HTC_CAM_START
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x86);
		break;
		case FL_MODE_CAM_FLASH_LEVEL2:
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x88);
		break;
		case FL_MODE_CAM_FLASH_LEVEL3:
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x8C);
		break;
		case FL_MODE_CAM_FLASH_LEVEL4:
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x90);
		break;
		case FL_MODE_CAM_FLASH_LEVEL5:
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x94);
		break;
		case FL_MODE_CAM_FLASH_LEVEL6:
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
			tps61310_i2c_command(0x01, 0x98);
		break;
		case FL_MODE_CAM_FLASH_LEVEL7:
			tps61310_i2c_command(0x05, 0x6A);
			tps61310_i2c_command(0x00, 0x00);
						...
						enum led_brightness brightness)
{
	...
	if (brightness > 0 && brightness <= LED_HALF) {
		...
		else if (brightness == (LED_CAMERA)){//HTC_CAM chuck add main-flash for camera disalbe timmer because of its implementation by ISP_FW.
			mode = FL_MODE_CAMERA_EFFECT_FLASH;
		} else if (brightness == (LED_HALF + 3))
			mode = FL_MODE_CAM_FLASH_LEVEL1;
		else if (brightness == (LED_HALF + 4))
			mode = FL_MODE_CAM_FLASH_LEVEL2;
		else if (brightness == (LED_HALF + 5))
			mode = FL_MODE_CAM_FLASH_LEVEL3;
		else if (brightness == (LED_HALF + 6))
			mode = FL_MODE_CAM_FLASH_LEVEL4;
		else if (brightness == (LED_HALF + 7))
			mode = FL_MODE_CAM_FLASH_LEVEL5;
		else if (brightness == (LED_HALF + 8))
			mode = FL_MODE_CAM_FLASH_LEVEL6;
		else if (brightness == (LED_HALF + 9))
			mode = FL_MODE_CAM_FLASH_LEVEL7;
		else
