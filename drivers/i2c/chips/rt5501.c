...
int debugfs_mode = 0;	//init mode
int debugfs_rc = 0;		// hs om
#endif
// HTC_AUD_END    Yangyi
...
static int rt5501_headset_detect(int on)
{
	...
	if(high_imp) {
                rt5501_write_reg(AMP_VOL,0x7);	// -35dB
                rt5501_write_reg(0xb1,0x81);
	}else if(MFG_MODE == 0){
		rt5501_write_reg(AMP_VOL,VOL_MUTE);	// if not MFG Mode, then mute the amp
	}
            last_spkamp_state = 0;
...
// HTC_AUD_ADD    Yangyi     for decrease the delay
        mdelay(20);
// HTC_AUD_END    Yangyi
        break;
...
static void hs_imp_detec_func(struct work_struct *work)
{
	...
	} /*if(temp[0] & RT5501_SENSE_READY)*/ else {
		if(hs->hs_qstatus == RT5501_QUERY_HEADSET)
...
static void volume_ramp_func(struct work_struct *work)
{
	...
	if(high_imp)
        ...
        rt5501_write_reg(0x2,0x0);	// Noise gate disable
        mdelay(1);
        val = 0x7;
...
static int rt5501_remove(struct i2c_client *client)
{
    ...
    //gpio_free(p6185data->gpio_rt5501_hp_en);
    kfree(p6185data);
