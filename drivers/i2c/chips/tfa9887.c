...
#define DEBUG (0)
#define TPA9887_WRITE_I2C	_IOW(TPA9887_IOCTL_MAGIC, 0x07, unsigned int)
...
unsigned char cf_dsp_bypass[4][3] = {
	// set up sample rate and channel
	{0x04, 0x88, 0x53},//for Rchannel
	// add adjust volume and amplifier PMW output
    {0x06, 0x00, 0x07},
	{0x09, 0x06, 0x19},
	{0x09, 0x06, 0x18}
};
...
void set_tfa9887_spkamp(int en, int dsp_mode)
{
	...
	AUD_INFO("%s: en = %d dsp_enabled = %d, last_spkamp_state=%d\n", __func__, en, dsp_enabled, last_spkamp_state);
	mutex_lock(&spk_amp_lock);
	if (en && !last_spkamp_state) {
		last_spkamp_state = 1;
		/* NXP CF DSP Bypass mode */
		if (dsp_enabled == 0) {
			AUD_INFO("set_tfa9887_spkamp dsp_enable =0 \n");
			for (i=0; i <4 ; i++)
			...
			AUD_INFO("set_tfa9887_spkamp dsp_enable =1 \n");
			...
			mute_data[0] = 0x6;
			mute_data[2] &= 0xef;  //bit 5 dn = un=mute
			power_data[0] = 0x9;
		...
		if (dsp_enabled == 0) {
			...
			tfa9887_i2c_read(mute_data + 1, 2);
			mute_data[0] = 0x6;
			mute_data[2] |= 0x10; //bit 5 up = mute
			power_data[0] = 0x9;
...
//		AUD_INFO("%s: TPA9887_WRITE_CONFIG\n", __func__);
		...
		break;
	//htc audio ++
	case TPA9887_WRITE_I2C:
		AUD_INFO("%s: TPA9887_WRITE_I2C for audioloop\n", __func__);
		...
		AUD_INFO("End TPA9887_WRITE_CONFIG\n");
