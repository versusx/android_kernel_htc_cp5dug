...
#define E(x...) printk(KERN_ERR "[COMP][AKM8963 ERROR] " x)
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		/* Platform data is not available.
		   Layout and Outbit information should be set
		   by each application. */
	...
	/***** IRQ setup *****/
    irq = gpio_to_irq(pdata->gpio_DRDY);
	s_akm->irq = irq;

	if (s_akm->irq == 0) {
		dev_info(&client->dev, "%s: IRQ is not set.", __func__);
		/* Use timer to notify measurement end */
		INIT_DELAYED_WORK(&s_akm->work, akm8963_delayed_work);
	} else {
		dev_info(&client->dev, "%s: IRQ = %d", __func__, s_akm->irq);
	...
	printk(KERN_INFO"%s:successfull!\n",__func__);
	return 0;
