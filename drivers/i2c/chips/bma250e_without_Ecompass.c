...
#define D(x...) printk(KERN_DEBUG "[GSNR][BMA250E NO_COMP] " x)
#define I(x...) printk(KERN_INFO "[GSNR][BMA250E NO_COMP] " x)
#define W(x...) printk(KERN_WARNING"[GSNR][BMA250E NO_COMP WARNING] " x)
#define E(x...) printk(KERN_ERR "[GSNR][BMA250E NO_COMP ERROR] " x)
#define DIF(x...) \
	if (debug_flag) \
		printk(KERN_DEBUG "[GSNR][BMA250E NO_COMP DEBUG] " x)
...
static void gsensor_poll_work_func(struct work_struct *work)
{
	...
	if (atomic_read(&a_flag)) {
		...
		for (j = 0; j < 3; j++) {
			for (k = 0; k < 3; k++){
				m_Alayout[j][k] = pdata->layouts[0][j][k];
			}
		}
