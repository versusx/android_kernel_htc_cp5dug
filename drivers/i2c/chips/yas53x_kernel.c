...
static int
yas53x_i2c_open(void)
{
	return 0;
}

static int
yas53x_i2c_close(void)
{
	return 0;
}

static int
yas53x_i2c_write(uint8_t addr, const uint8_t *buf, int len)
{
	uint8_t tmp[16];
	if (sizeof(tmp) - 1 < len)
		return -1;
...
static int
yas53x_i2c_read(uint8_t addr, uint8_t *buf, int len)
{
...
static void
yas53x_msleep(int ms)
{
	usleep_range(ms * 1000, (ms + 1) * 1000);
}

static void
yas53x_current_time(uint32_t *msec)
{
...
static int
yas53x_enable(struct yas53x_data *data)
{
...
static int
yas53x_disable(struct yas53x_data *data)
{
...
/* Sysfs interface */
static ssize_t
yas53x_delay_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
...
static ssize_t
yas53x_delay_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t
yas53x_enable_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
...
static ssize_t
yas53x_enable_store(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
	long value;
...
static ssize_t
yas53x_position_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
	int position;
...
static ssize_t
yas53x_position_store(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
	long value;
...
static ssize_t
yas53x_data_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
	int32_t last[3], i;
...
static ssize_t
yas53x_offset_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
	int8_t hard_offset[3];
...
static ssize_t
yas53x_offset_store(struct device *dev,
...
static ssize_t
yas53x_self_test_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
...
static ssize_t
yas53x_self_test_noise_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct yas53x_data *data = i2c_get_clientdata(this_client);
...
static DEVICE_ATTR(position, S_IRUGO|S_IWUSR,
	yas53x_position_show, yas53x_position_store);
static DEVICE_ATTR(offset, S_IRUGO|S_IWUSR,
	yas53x_offset_show, yas53x_offset_store);
...
static void
yas53x_input_work_func(struct work_struct *work)
{
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void
yas53x_early_suspend(struct early_suspend *h)
{
	struct yas53x_data *data =
		container_of(h, struct yas53x_data, sus);
...
static void
yas53x_late_resume(struct early_suspend *h)
{
	struct yas53x_data *data =
		container_of(h, struct yas53x_data, sus);
	if (atomic_read(&data->enable))
...
static int
yas53x_suspend(struct i2c_client *client, pm_message_t mesg)
{
...
static int
yas53x_resume(struct i2c_client *client)
{
	struct yas53x_data *data = i2c_get_clientdata(client);
	if (atomic_read(&data->enable))
		schedule_delayed_work(&data->work, 0);
...
static int
yas53x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[YAMAHA]compass probe start\n");
	if (data == NULL) {
		rt = -ENOMEM;
		goto err;
	}
	...
	if (input_data == NULL) {
		rt = -ENOMEM;
		printk(KERN_ERR "Failed to allocate input_data device\n");
		goto err;
	}
	...
	if (rt) {
		printk(KERN_ERR "Unable to register input_data device: %s\n",
				input_data->name);
		goto err;
	}
	data_registered = 1;
	...
	if (rt) {
		printk(KERN_ERR "yas53x_probe: sysfs_create_group failed[%s]\n",
					YAS53X_NAME);
		goto err;
	}
	sysfs_created = 1;
	...
	if (client->dev.platform_data != NULL)
		chip_layout = *(int *)client->dev.platform_data;
	this_drv.set_position(chip_layout);
	this_drv.get_offset(data->hard_offset);
	i2c_set_clientdata(client, data);
	printk("[YAMAHA]compass probe end\n");
	return 0;
...
static int
yas53x_remove(struct i2c_client *client)
{
	struct yas53x_data *data = i2c_get_clientdata(client);
	if (data != NULL) {
#ifdef CONFIG_HAS_EARLYSUSPEND
...
static struct i2c_device_id yas53x_idtable[] = {
	{YAS53X_NAME, 0},
	{}
};
MODULE_DEVICE_TABLE(i2c, yas53x_idtable);
