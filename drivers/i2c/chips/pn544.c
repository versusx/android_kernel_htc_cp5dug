...
#define E(x...) printk(KERN_ERR "[NFC] NFC_ERR " x)
		...
		const struct i2c_device_id *id)
{
	...
	if (is_alive)
	{
		...
		client->irq = gpio_to_irq(pni->irq_gpio);
 ...
 */
#if 1
static void __devinit pn544_init_async(void *unused, async_cookie_t cookie)
{
	i2c_add_driver(&pn544_driver);
}

static int __devinit pn544_dev_init(void)
{
	printk("0521: %s: Loading pn544 driver\n", __func__);
	async_schedule(pn544_init_async, NULL);
	return 0;
}

static void __exit pn544_dev_exit(void)
{
	printk("%s: Unloading pn544 driver\n", __func__);
