...
static int bma250_set_mode(struct i2c_client *client, unsigned char Mode)
{
    ...
    if(cir_flag && Mode == BMA250_MODE_SUSPEND) {
		I("%s:Enter CIR_ALWAYS_READY Mode.",__func__);
        return 0;
    }else{
#endif
	...
	if (Mode < 3) {
		...
		I("%s:Mode=%d,cir_flag = %d(PWK:%d)",__func__,Mode,cir_flag,power_key_pressed);
		switch (Mode) {
		...
		const char *buf, size_t count)
{
	...
	if(enable == 1 && !power_key_pressed){ 
	    ...
	    while(retry <= 30) {
			...
			BMA250_INT_MODE_SEL__REG, &reg_val);
			if (reg_val & 0x01) { 
				I("%s:Int Mode set done. (Reg0x%X=0x%x).",__func__,BMA250_INT_MODE_SEL__REG,reg_val);
				break;
			} else {
				I("%s:Set Int Mode retry(Reg0x%X=0x%x)---%d",__func__,BMA250_INT_MODE_SEL__REG,reg_val,retry);
				retry++;
			}
			if (retry >= 30)
				E("%s:Int Mode set Fail!",__func__);
		}
	    ...
	    while(retry <= 30) {
			...
			comres = bma250_smbus_read_byte(bma250->bma250_client,
				BMA250_INT_ENABLE1_REG, &reg_val);
			if ((reg_val & 0x03) == 0x03) { 
				I("%s:Int enable set done. (Reg0x%X=0x%x).",__func__,BMA250_INT_ENABLE1_REG,reg_val);
				break;
			} else {
				I("%s:Set Int enable retry(Reg0x%X=0x%x)---%d",__func__,BMA250_INT_ENABLE1_REG,reg_val,retry);
				retry++;
			}
			if (retry >= 30)
				E("%s:Int enable set Fail!",__func__);
	    }
	    ...
	    while(retry <= 30) {
			...
			comres = bma250_smbus_read_byte(bma250->bma250_client,
			BMA250_EN_INT1_PAD_SLOPE__REG, &reg_val);
			if (reg_val & 0x04) { 
				I("%s:Int PAD set done. (Reg0x%X=0x%x).",__func__,BMA250_EN_INT1_PAD_SLOPE__REG,reg_val);
				break;
			} else {
				I("%s:Set Int PAD retry(Reg0x%X=0x%x)---%d",__func__,BMA250_EN_INT1_PAD_SLOPE__REG,reg_val,retry);
				retry++;
			}
			if (retry >= 30)
				E("%s:Int PAD set Fail!",__func__);
		}

	    error += bma250_set_mode(bma250->bma250_client, BMA250_MODE_NORMAL);

	    if (error)
			E("Always Ready Fail(%d) \n",error);
		return error;
		...
		const struct i2c_device_id *id)
{
	...
	ret = gpio_request(data->pdata->intr, "gpio_bma250_intr");
	if (ret < 0)
		E("request gpio fail.\n");

	ret = gpio_direction_input(data->pdata->intr);

	irq = gpio_to_irq(data->pdata->intr);
	err = request_any_context_irq(irq, bma250_irq_handler, IRQF_TRIGGER_RISING|IRQF_NO_SUSPEND,
			"bma250", data);
    enable_irq_wake(data->IRQ);
	if (err)
		E("could not request irq,irq = %d\n",irq);
...
static int bma250_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (cir_flag == 1) {
		bma250_smbus_read_byte(client, BMA250_INT_MODE_SEL__REG, reg);
		bma250_smbus_read_byte(client, BMA250_INT_ENABLE1_REG, reg+1);
		bma250_smbus_read_byte(client, BMA250_EN_INT1_PAD_SLOPE__REG, reg+2);
		bma250_smbus_read_byte(client, BMA250_EN_LOW_POWER__REG, reg+3);
		I("CIR MODE: Reg0x%x(1):%d, Reg0x%x(3):%d, Reg0x%x(4):%d, Reg0x%x(0):%d", BMA250_INT_MODE_SEL__REG, reg[0],
			BMA250_INT_ENABLE1_REG, reg[1], BMA250_EN_INT1_PAD_SLOPE__REG, reg[2], BMA250_EN_LOW_POWER__REG, reg[3]);
	}
