...
struct synaptics_ts_data {
	...
	struct work_struct  psensor_work_enable;
#endif
...
static int i2c_syn_error_handler(struct synaptics_ts_data *ts, uint8_t reset, char *reason, const char *fun_name)
{
	...
	if (reset) {
		if (ts->power) {
			...
			gpio_request(ts->gpio_reset, "touch reset");
...
static int syn_config_update(struct synaptics_ts_data *ts, int attr)
{
	...
	if (ts->config == NULL){
		printk("[TP] ts->config is NULL, error");
		return ENODEV;
	}
	crc_checksum = syn_crc((uint16_t *)ts->config, SYN_CONFIG_SIZE / 2 - 2);
...
{
	int ret;
	uint8_t buf[ts->finger_support * 8], noise_index[10], noise_state = 0, finger_num[3];
	static int x_pos[10] = {0}, y_pos[10] = {0};
	int reg_addr = 0;
	uint8_t *p_buf_addr = NULL;

	memset(buf, 0x0, sizeof(buf));
	memset(noise_index, 0x0, sizeof(noise_index));
	memset(finger_num, 0x0, sizeof(finger_num));
	if (ts->package_id < 3400){
		ret = i2c_syn_read(ts->client,get_address_base(ts, 0x11, DATA_BASE), finger_num, 3);
	} else {
		ret = i2c_syn_read(ts->client,
	...
	if (ret < 0) {
		...
		if (ts->debug_log_level & BIT(0)) {
			if (ts->package_id < 3400) {
				printk(KERN_INFO "[TP] finger number data:");
				for (i = 0; i < 3; i++)
					printk(KERN_INFO " %2x", finger_num[i]);
				printk(KERN_INFO "\n");
			}else {
		...
		for (i = 0; i < ts->finger_support; i++) {
			...
			if (ts->package_id < 3400)
				finger_state = finger_num[(i / 4)] >> ((i * 2) % 8);
			else
		...
		if (ts->package_id < 3400){
			for (i = 0; i < ts->finger_support; i++) {
				if ((finger_pressed & BIT(i)) == BIT(i)){
					reg_addr = (get_address_base(ts, 0x11, DATA_BASE) + 3) + i*5;
					p_buf_addr = buf + i*5;
					ret = i2c_syn_read(ts->client, reg_addr, p_buf_addr, 5);
					if (ret < 0){
						break;
					}
				}
		...
		if ((ts->debug_log_level & BIT(0)) && (ts->package_id < 3400)) {
			printk(KERN_INFO "[TP] Touch:");
			for (i = 0; i < (ts->finger_count*5); i++)
				printk(KERN_INFO " %2x", buf[i]);
			printk(KERN_INFO "\n");
		}
...
static void synaptics_ts_close_psensor_func(struct work_struct *work)
{
	...
	if(ts->psensor_resume_enable == 1) {
		...
		psensor_enable_by_touch_driver(0);
		ts->psensor_resume_enable = 0;
		printk(KERN_INFO "[TP] %s: exit\n", __func__);
	}
}

#ifndef CONFIG_MACH_DUMMY
static void synaptics_ts_enable_psensor_func(struct work_struct *work)
{
	printk(KERN_INFO "[TP] %s: enter\n", __func__);
...
int   get_tp_sensor_id()
{
	printk(KERN_INFO "[TP] %s: touch panel sensor id is : %x\n", __func__, tp_sensor_id);
	return tp_sensor_id;
}
...
static int syn_probe_init(void *arg)
{
...
#ifdef MFG_KERNEL_TOUCH_FW_UPDATE
...
#ifndef CONFIG_MACH_DUMMY
		INIT_WORK(&ts->psensor_work_enable, synaptics_ts_enable_psensor_func);
#endif
	...
	ts->client->irq = gpio_to_irq(pdata->gpio_irq);
...
static int synaptics_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(ts->psensor_detection) {
		if(ts->psensor_resume_enable == 1){
			...
			ts->psensor_resume_enable = 0;
#else
			printk(KERN_INFO "[TP] %s: Disable psensor by touch workqueue\n", __func__);
			queue_work(ts->syn_psensor_wq, &ts->psensor_work);
#endif
		}
	}
...
static int synaptics_ts_resume(struct i2c_client *client)
{
	...
	if (ts->psensor_detection) {
		if(ts->psensor_status == 0) {
			...
			psensor_enable_by_touch_driver(1);
#else
			printk(KERN_INFO "[TP] %s: Enable psensor by touch workqueue\n", __func__);
			queue_work(ts->syn_psensor_wq, &ts->psensor_work_enable);
#endif
		}
