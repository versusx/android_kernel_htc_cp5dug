...
#define GPIO_LCD_1V8    182
#define GPIO_LCD_3V0    214
...
int tiger_enable_gpio(struct panel_spec *panel)
{
	int ret = 0, lcm_id0 = 0, lcm_id1 = 0;
	...
	gpio_request(GPIO_LCD_3V0, "LCD_3V0");
	...
	gpio_direction_output(GPIO_LCD_3V0, 0);
	gpio_direction_output(GPIO_LCD_RST, 0);

	mdelay(10); //after rst

	printk("[DISP] Enable LCM 1V8\n");
	gpio_set_value(GPIO_LCD_1V8, 1);

/*
	lcm_id0 = gpio_get_value(GPIO_LCD_ID0);
	printk("[DISP] LCM_ID0=%d\n", lcm_id0);
	mdelay(10);

	lcm_id1 = gpio_get_value(GPIO_LCD_ID1);
	printk("[DISP] LCM_ID1=%d\n", lcm_id1);
	mdelay(10);
	...
	if ((lcm_id0 == 1) && (lcm_id1 == 0)){
		panel->width  = 480;
		panel->height = 800;
		lcm_vendor = NOVATEK_NT35512;
		printk("[DISP] lcm_vendor = NOVATEK_NT35512\n");
	} else {
		panel->width  = 540;
		panel->height = 960;
		lcm_vendor = NOVATEK_NT35517;
		printk("[DISP] lcm_vendor = NOVATEK_NT35517\n");
	}
*/
