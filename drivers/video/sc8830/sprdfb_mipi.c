...
extern int32_t sprdfb_dsi_enter_ulps(struct sprdfb_device *dev);
  /* rgb_calc_h_timing/rgb_calc_v_timing
  * Need to sync rgb_calc_h_timing/rgb_calc_v_timing from sprdfb_rgb.c, Because don't need to compile the sprdfb_rgb.c
  * Chaoyi.hu
  */
...
static void mipi_dispc_init_config(struct panel_spec *panel)
{
	...
	if(NULL == panel){
		printk(KERN_ERR "[DISP] sprdfb: [%s] fail.(Invalid Param)\n", __FUNCTION__);
		return;
	}

	if(SPRDFB_PANEL_TYPE_MIPI != panel->type){
		printk(KERN_ERR "[DISP] sprdfb: [%s] fail.(not  mcu panel)\n", __FUNCTION__);
		return;
	}
	...
	pr_debug("[DISP] sprdfb: [%s] DISPC_DPI_CTRL = %d\n", __FUNCTION__, dispc_read(DISPC_DPI_CTRL));
}
...
{
	if(NULL == panel){
		printk("[DISP] sprdfb: [%s] fail. (Invalid param)\n", __FUNCTION__);
		return false;
	}

	if(SPRDFB_PANEL_TYPE_MIPI != panel->type){
		printk("[DISP] sprdfb: [%s] fail. (not mipi param)\n", __FUNCTION__);
		return false;
	}
...
static void sprdfb_mipi_panel_mount(struct sprdfb_device *dev)
{
	...
	pr_debug(KERN_INFO "[DISP] sprdfb: [%s], dev_id = %d\n",__FUNCTION__, dev->dev_id);
