...
static irqreturn_t dsi_isr0(int irq, void *data)
{
	...
	printk(KERN_ERR "[DISP] sprdfb: [%s](0x%x)!\n", __FUNCTION__, reg_val);
...
static irqreturn_t dsi_isr1(int irq, void *data)
{
	...
	struct sprdfb_dsi_context *dsi_ctx1 = (struct sprdfb_dsi_context *)data;
	struct sprdfb_device *dev = dsi_ctx1->dev;
	dsih_ctrl_t* dsi_instance = &(dsi_ctx1->dsi_inst);
	...
	if(BIT(7) == (reg_val & BIT(7))){
		...
		/*need delay 1us*/
		printk("[DISP] sprdfb: reset dsi host!\n");
...
static int32_t dsi_edpi_setbuswidth(struct info_mipi * mipi)
{
	...
	case 24:
		...
		break;
	default:
		printk(KERN_ERR "[DISP] sprdfb:[%s] fail, invalid video_bus_width\n", __FUNCTION__);
		return 0;
	}
...
int32_t dsi_dpi_init(struct sprdfb_device *dev)
{
	...
	if(result != OK){
		printk(KERN_ERR "[DISP] sprdfb: [%s] mipi_dsih_dpi_video fail (%d)!\n", __FUNCTION__, result);
		return -1;
	}
...
static int32_t dsi_module_init(struct sprdfb_device *dev)
{
	...
	if(dsi_ctx.is_inited){
		printk(KERN_INFO "[DISP] sprdfb: dsi_module_init. is_inited==true!");
		return 0;
	}
	else{
		printk(KERN_INFO "[DISP] sprdfb: dsi_module_init. call only once!");
	}
	...
	if (ret) {
		printk(KERN_ERR "[DISP] sprdfb: dsi failed to request irq int0!\n");
//		clk_disable(dsi_ctx.clk_dsi);
	}else{
		printk(KERN_ERR "[DISP] sprdfb: dsi request irq int0 OK!\n");
	}

	ret = request_irq(IRQ_DSI_INTN1, dsi_isr1, IRQF_DISABLED, "DSI_INT1", &dsi_ctx);
	if (ret) {
		printk(KERN_ERR "[DISP] sprdfb: dsi failed to request irq int1!\n");
//		clk_disable(dsi_ctx.clk_dsi);
	}else{
		printk(KERN_ERR "[DISP] sprdfb: dsi request irq int1 OK!\n");
	}
...
int32_t sprdfb_dsi_uninit(struct sprdfb_device *dev)
{
	...
	if(OK != result){
		printk(KERN_ERR "[DISP] sprdfb: [%s]: sprdfb_dsi_uninit fail (%d)!\n", __FUNCTION__, result);
		return -1;
	}
...
static int32_t sprdfb_dsi_set_data_lp_mode(void)
{
	...
	dsi_core_write_function(SPRD_MIPI_DSIC_BASE, R_DSI_HOST_ERROR_MSK0, 0x1fffff); //add for debug
...
static int32_t sprdfb_dsi_set_data_hs_mode(void)
{
	...
	pr_debug(KERN_INFO "sprdfb: [%s] set data hs mode active_mode=%d\n", __FUNCTION__, active_mode);
...
static int32_t sprdfb_dsi_gen_write(uint8_t *param, uint16_t param_length)
{
	...
	if(OK != result){
		printk(KERN_ERR "[DISP] sprdfb: [%s] error (%d)\n", __FUNCTION__, result);
		return -1;
	}
	return 0;
}
...
static int32_t sprd_dsi_force_read(uint8_t command, uint8_t bytes_to_read, uint8_t * read_buffer)
{
	...
	if(0 != (reg_val & 0x2)){
		printk("[DISP] sprdfb: [%s] mipi read hang (0x%x)!\n", __FUNCTION__, reg_val);
		dsi_ctx.status = 2;
		iRtn = 0;
	}

	if(0 != (reg_val_1 & 0x701)){
		printk("[DISP] sprdfb: [%s] mipi read status error!(0x%x, 0x%x)\n", __FUNCTION__, reg_val_1, reg_val_2);
		iRtn = 0;
	}
