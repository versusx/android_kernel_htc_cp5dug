...
dsih_error_t mipi_dsih_open(dsih_ctrl_t * instance)
{
    ...
    else
    {
        ...
        if (i >= mipi_dsih_no_of_versions)
        {
            ...
            {
                instance->log_info("[DISP] driver does not support this core version 0x%lX", version);
            }
    ...
    mipi_dsih_hal_dcs_wr_tx_type(instance, 3, 1); 
    ...
    mipi_dsih_hal_gen_wr_tx_type(instance, 3, 1); 
    ...
    mipi_dsih_hal_tx_escape_division(instance, 4); 
#else
    mipi_dsih_hal_tx_escape_division(instance, 3); 
#endif
...
void mipi_dsih_allow_return_to_lp(dsih_ctrl_t * instance, int hfp, int hbp, int vactive, int vfp, int vbp, int vsync)
{
    ...
    {
        instance->log_error("[DISP] invalid instance");
    }
}
void mipi_dsih_dcs_cmd_lp_transmission(dsih_ctrl_t * instance, int long_write, int short_write, int short_read)
{
    ...
    if (instance->status == INITIALIZED)
    {
        ...
        mipi_dsih_hal_dcs_wr_tx_type(instance, 3, long_write); 
...
void mipi_dsih_gen_cmd_lp_transmission(dsih_ctrl_t * instance, int long_write, int short_write, int short_read)
{
    ...
    if (instance->status == INITIALIZED)
    {
        ...
        mipi_dsih_hal_gen_wr_tx_type(instance, 3, long_write); 
...
dsih_error_t mipi_dsih_dpi_video(dsih_ctrl_t * instance, dsih_dpi_video_t * video_params)
{
    ...
    switch (video_params->color_coding)
    {
        ...
        default:
            if (instance->log_error != 0)
            {
                instance->log_error("[DISP] invalid color coding");
            }
	...
	mipi_dsih_hal_dpi_lp_during_hbp(instance, 0);
        ...
        if (instance->log_info != 0)
        {
            instance->log_info("[DISP] burst video");
            instance->log_info("[DISP] h line time %ld", (uint16_t)((video_params->h_total_pixels * ratio_clock_xPF) / PRECISION_FACTOR));
            instance->log_info("[DISP] video_size %ld", video_size);
        }
#endif
    }
    else
    {   
        ...
        if (total_bytes >= bytes_per_chunk)
        {
            ...
            {   
                ...
                if (instance->log_info != 0)
                {
                    instance->log_info("[DISP] no multi no null video");
                    instance->log_info("[DISP] h line time %ld", (uint16_t)((video_params->h_total_pixels * ratio_clock_xPF) / PRECISION_FACTOR));
                    instance->log_info("[DISP] video_size %ld", video_size);
                }
        ...
        {
            instance->log_error("[DISP] resolution cannot be sent to display through current settings");
    ...
    if (instance->log_info != 0)
    {
        instance->log_info("[DISP] total_bytes %d", total_bytes);
        instance->log_info("[DISP] bytes_per_chunk %d", bytes_per_chunk);
        instance->log_info("[DISP] bytes left %d", bytes_left);
        instance->log_info("[DISP] null packets %d", null_packet_size);
        instance->log_info("[DISP] chunks %ld", no_of_chunks);
        instance->log_info("[DISP] video_size %ld", video_size);
    }
...
dsih_error_t mipi_dsih_dcs_wr_cmd(dsih_ctrl_t * instance, uint8_t vc, uint8_t* params, uint16_t param_length)
{
    ...
    switch (params[i])
    {
        ...
        default:
            if (instance->log_error != 0)
            {
                instance->log_error("[DISP] invalid DCS command");
            }
...
dsih_error_t mipi_dsih_gen_wr_packet(dsih_ctrl_t * instance, uint8_t vc, uint8_t data_type, uint8_t* params, uint16_t param_length)
{
    ...
    {   
        ...
        if ((param_length - 2) < word_count)
        {
            if (instance->log_error != 0)
            {
                instance->log_error("[DISP] sent > input payload. complemented with zeroes");
            }
        ...
        else if ((param_length - 2) > word_count)
        {
            if (instance->log_error != 0)
            {
                instance->log_error("[DISP] Overflow - input > sent. payload truncated");
            }
        }
...
uint16_t mipi_dsih_gen_rd_packet(dsih_ctrl_t * instance, uint8_t vc, uint8_t data_type, uint8_t msb_byte, uint8_t lsb_byte, uint8_t bytes_to_read, uint8_t* read_buffer)
{
    ...
    if (!(timeout < DSIH_FIFO_ACTIVE_WAIT))
    {
        if (instance->log_error != 0)
        {
            instance->log_error("[DISP] tx rd command timed out");
        }
        return 0;
    }
    ...
    {   
        if (!mipi_dsih_hal_gen_rd_cmd_busy(instance))
        {
            ...
            else
            {
                if (instance->log_error != 0)
                {
                    instance->log_error("[DISP] rx buffer empty");
                }
                return 0;
            }
        }
    ...
    if (instance->log_error != 0)
    {
        instance->log_error("[DISP] rx command timed out");
    }
    return 0;
}
uint32_t mipi_dsih_dump_register_configuration(dsih_ctrl_t * instance, int all, register_config_t *config, uint16_t config_length)
{
    ...
    {   
        for (current = R_DSI_HOST_VERSION; current <= R_DSI_HOST_ERROR_MSK1; count++, current += (R_DSI_HOST_PWR_UP - R_DSI_HOST_VERSION))
        {
            ...
            {   
                if (instance->log_info != 0)
                {
                    instance->log_info("[DISP] DSI 0x%lX:0x%lX", current, mipi_dsih_read_word(instance, current));
                }
            }
            else
            {
    ...
    else
    {
        if(config == 0)
        {
            if (instance->log_error != 0)
            {
                instance->log_error("[DISP] invalid buffer");
            }
        }
        else
        {
