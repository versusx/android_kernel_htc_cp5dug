...
extern void dfs_request_bw_timeout(unsigned int req_bw);
enum mali_pm_scheme
{
...
static enum mali_pm_level next_level_dynamic = MALI_PM_LEVEL_2_STANDBY; 
...
void mali_pm_core_event(enum mali_core_event core_event)
{
	...
	{
		mali_pm_event(MALI_PM_EVENT_CORES_WORKING, MALI_FALSE, 0); 
		dfs_request_bw_timeout(400000);
		if(!init_dfs_version) {
			printk("[DFS]new dfs version is v10\n");
			init_dfs_version = 1;
		}
	}
	else if (transition_idle == MALI_TRUE)
	{
		mali_pm_event(MALI_PM_EVENT_CORES_IDLE, MALI_FALSE, 0); 
	}
...
void mali_pm_os_suspend(void)
{
	...
	mali_pm_event(MALI_PM_EVENT_OS_SUSPEND, MALI_FALSE, 0); 
...
void mali_pm_os_resume(void)
{
	...
	mali_pm_event(MALI_PM_EVENT_OS_RESUME, MALI_FALSE, 0); 
