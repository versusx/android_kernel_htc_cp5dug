...
_mali_osk_errcode_t mali_platform_power_mode_change(mali_power_mode power_mode)
{
	...
	case MALI_POWER_MODE_ON:
		if((0!=gpu_power_on)||(0!=gpu_clock_on))
		{
			MALI_DEBUG_PRINT(2,("MALI_POWER_MODE_ON gpu_power_on:%d  gpu_clock_on:%d power_reg:0x%x\n",gpu_power_on,gpu_clock_on,sci_glb_read(REG_PMU_APB_PD_GPU_TOP_CFG,0xFFFFFFFF)));
			//gpu_power_on=0;
			//gpu_clock_on=0;
		}
		if(!gpu_power_on)
