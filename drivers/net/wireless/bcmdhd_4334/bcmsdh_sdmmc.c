...
sdioh_attach(osl_t *osh, void *bar0, uint irq)
{
	...
	if (gInstance == NULL) {
		printk("[WLAN] %s: SDIO Device not present\n", __FUNCTION__);
		return NULL;
	}

	if ((sd = (sdioh_info_t *)MALLOC(osh, sizeof(sdioh_info_t))) == NULL) {
		printk("[WLAN] sdioh_attach: out of memory, malloced %d bytes\n", MALLOCED(osh));
		return NULL;
	}
	bzero((char *)sd, sizeof(sdioh_info_t));
	sd->osh = osh;
	if (sdioh_sdmmc_osinit(sd) != 0) {
		printk("[WLAN] %s:sdioh_sdmmc_osinit() failed\n", __FUNCTION__);
...
sdioh_request_byte(sdioh_info_t *sd, uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	if (!gInstance->func[func]) {
		printk("[WLAN]func %d at %s is null\n", func, __func__);
		return SDIOH_API_RC_FAIL;
	}
			...
			if (regaddr == SDIOD_CCCR_IOEN) {
				if (gInstance->func[2]) {
					...
					if (*byte & SDIO_FUNC_ENABLE_2) {
						...
						if (err_ret) {
							printk("[WLAN]bcmsdh_sdmmc: enable F2 failed:%d",err_ret);
						}
					} else {
						
						err_ret = sdio_disable_func(gInstance->func[2]);
						if (err_ret) {
							printk("[WLAN]bcmsdh_sdmmc: Disab F2 failed:%d",err_ret);
						}
					}
					sdio_release_host(gInstance->func[2]);
				}
	...
	if (err_ret) {
		...
		if (err_ret == -(ETIMEDOUT+50)) {
			...
			if (cmd_timeout_counter >= MAX_CMD_TIMEOUT_RETRY) {
				...
				printk("[WLAN]SDIO TIMEOUT trigged by wait for completion timeout! SDIO Controller something wrong!\n");
				ndev = wl_get_dev();
                                   ...
                                   uint32 *word, uint nbytes)
{
	...
	if (err_ret) {
		printk("[WLAN]bcmsdh_sdmmc: Failed to %s word, Err: 0x%08x",rw ? "Write" : "Read", err_ret);
	}
                     ...
                     uint addr, void *pkt)
{
	...
	if (0 != dma_len) {
		...
		for (pnext = pkt;
			...
			if (SGCount >= SDIOH_SDMMC_MAX_SG_ENTRIES) {
				printk("[WLAN] %s: sg list entries exceed limit\n",__FUNCTION__);
			return (SDIOH_API_RC_FAIL);
			}
		}
		...
		if (0 != err_ret) {
			printk("[WLAN] %s:CMD53 %s failed with code %d\n",__FUNCTION__,write ? "write" : "read",err_ret);			
                        printk("[WLAN] %s:Disabling rxchain and fire it with PIO\n",__FUNCTION__);
			sd->use_rxchain = FALSE;
			pkt = pprev;
			lft_len = ttl_len;
		} else if (!fifo) {
	...
	if (0 != lft_len) {
		...
		for (pnext = pkt; pnext; pnext = PKTNEXT(sd->osh, pnext)) {
			...
			if (err_ret)
			  {
printk("[WLAN]%s: %s FAILED %p[%d], addr=0x%05x, pkt_len=%d, ERR=%d\n",
				       __FUNCTION__,
				       (write) ? "TX" : "RX",
                     ...
                     uint addr, uint reg_width, uint buflen_u, uint8 *buffer, void *pkt)
{
	...
	if (!gInstance->func[func]) {
		printk("[WLAN]func %d at %s is null(SDIOH_API_RC_FAIL)\n", func, __func__);
		return SDIOH_API_RC_FAIL;
	}
...
#endif 
			printk("[WLAN] %s: PKTGET failed: len %d (SDIOH_API_RC_FAIL)\n",__FUNCTION__, buflen_u);
			return SDIOH_API_RC_FAIL;
		}
