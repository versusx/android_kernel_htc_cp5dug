...
static int b_mode_set_frameburst = 0;
static s32
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		if (err) {
			...
			WL_DBG(("Rate %d Mbps\n", (rate / 2)));
			frameburst = ((rate/2) > 11)?0:1;
			if ((b_mode_set_frameburst == 0) && ((rate/2) <= 11)){
				printk("[WLAN] set frameburst, Rate is %d Mbps\n", (rate / 2)); 
				b_mode_set_frameburst = 1;
	...
	case WLC_E_PRUNE:
		...
		case WLC_E_LOAD_IND:
            ...
            if (status) {
                    ...
                    /* do NOT release CPU resources for Metrico lab */
#else
                    printf("LOW INDICATE!!\n");
				    wlan_unlock_perf();
					hotspot_high_ind = 0;
