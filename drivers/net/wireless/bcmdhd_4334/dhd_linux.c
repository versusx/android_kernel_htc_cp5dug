...
#ifndef HTC_SPRD_PLATFORM
	}  else if  (dhdcdc_power_active_while_plugin ) {
...
#else
#define TRAFFIC_HIGH_WATER_MARK                (670 *(6000/5000))
#define TRAFFIC_LOW_WATER_MARK          280 * (6000/5000)
#define TRAFFIC_S_HIGH_WATER_MARK	((TRAFFIC_HIGH_WATER_MARK) * 20)
#endif
...
fail_1:
	...
	if (!retry) {
      ...
      if(error)
      	{	
      		printk("[WLAN] error value %d, and release perf-lock\n",error);
      		wl_android_exit();
      	}
	return error;
}
