...
//HTC_AUD_START modify for voip support
#define  SAUDIO_STREAM_BLOCK_COUNT	8
				       ...
				       int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		mutex_lock(&dev_ctrl->mutex); //HTC_AUD modify for voip support
		saudio_send_common_cmd(dev_ctrl->dst, dev_ctrl->channel,
				       SAUDIO_CMD_START, stream->stream_id);
		mutex_unlock(&dev_ctrl->mutex); //HTC_AUD modify for voip support
...
static struct snd_saudio *saudio_card_probe(struct saudio_init_data *init_data)
{
	...
	result = snd_card_create(SNDRV_DEFAULT_IDX1, init_data->name, THIS_MODULE,
				 sizeof(struct snd_saudio), &saudio_card);  //HTC_AUD modify for voip support
					    ...
					    struct saudio_msg *msg)
{
	...
	if(stream->channel == 14)
		pr_debug("saudio.c: cur_blk_count is %d\n", cur_blk_count);
	elapsed_blks =
	...
	if (stream->periods_tosend) {
		while (stream->periods_tosend) {
			...
			if (result) {
				ETRACE
				    ("saudio.c: sblock get failed result is %d,dst is %d,channel is %d,periods_tosend is %d ,cur_blk_count is %d,last_elapsed_count is %d, last_getblk is %d\n",
				     result, stream->dst, stream->channel,
				     stream->periods_tosend,cur_blk_count,stream->last_elapsed_count,stream->last_getblk_count);
				break;
			}
			ETRACE("sblock: get ok");
			stream->last_getblk_count++;
			common = (struct cmd_common *)blk.addr;
			blk.length =
