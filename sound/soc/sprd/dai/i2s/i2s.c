...
//HTC_AUD_START, fei_tong modify for i2s suspend/resume
...
static pinmap_t i2s_on_pinmap[] = {
	...
	{REG_PIN_TRACEDAT3, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_OE},//GPIO 235
	{REG_PIN_TRACEDAT4, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(0)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_Z},//GPIO 236
	{REG_PIN_TRACEDAT5, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(0)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_Z},//GPIO 237
	{REG_PIN_TRACEDAT6, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(0)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_Z},//GPIO 238
};

static pinmap_t i2s_off_pinmap[] = {

	//bt i2s
	{REG_PIN_IIS0DI,                  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE},//GPIO 210
	{REG_PIN_IIS0DO,             	  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_NUL|BIT_PIN_SLP_WPD|BIT_PIN_SLP_OE},//GPIO 211
	{REG_PIN_IIS0CLK,                 BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_OE},//GPIO 212
	{REG_PIN_IIS0LRCK,                BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_OE},//GPIO 213

	//fm i2s
	{REG_PIN_TRACEDAT3, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_NUL|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE},//GPIO 235
	{REG_PIN_TRACEDAT4, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_OE},//GPIO 236
	{REG_PIN_TRACEDAT5, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_OE},//GPIO 237
	{REG_PIN_TRACEDAT6, 		  BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_OE},//GPIO 238
};
//HTC_AUD_END
	...
	case 32000:
		{
			...
			clk_put(clk_128m);
		printk("eleven: configure i2s : %d clock\n", config->fs);
		}
		break;
	case 48000:
		{
			struct clk *clk_76m8;
			clk_76m8 = clk_get(NULL, "clk_76m8");
			if (IS_ERR(clk_76m8)) {
				int ret = PTR_ERR(clk_76m8);
				pr_err("i2s get clock source error %d!\n", ret);
				return ret;
			}
			clk_set_parent(i2s->i2s_clk, clk_76m8);
			source_clk = clk_get_rate(clk_76m8);
			clk_set_rate(i2s->i2s_clk, source_clk);
			clk_put(clk_76m8);
		printk("eleven: configure i2s : %d clock\n", config->fs);
		}
...
//HTC_AUD_START, fei_tong modify for i2s suspend/resume
static void i2s_pin_config(pinmap_t *pinmap,int len)
{
	printk("Entering func:%s,len=%d\n", __func__, len);
	int i;
	for(i=0;i<len;i++){
...
static int config_i2s(void)
{
	i2s_pin_config(i2s_on_pinmap,ARRAY_SIZE(i2s_on_pinmap));
	return 0;
}

static int deconfig_i2s(void)
{
	i2s_pin_config(i2s_off_pinmap,ARRAY_SIZE(i2s_off_pinmap));
	return 0;
}

static int i2s_drv_suspend(struct platform_device *pdev)
{
	i2s_dbg("Entering func:%s,id =%d, name =%s\n", __func__,pdev->id, pdev->name);
	if(deconfig_i2s() < 0){
		i2s_dbg("error occured when deinitt");
		return -1;
	}
	return 0;
}

static int i2s_drv_resume(struct platform_device *pdev)
{
	i2s_dbg("Entering func: %s, name =%s, id =%d\n", __func__, pdev->name, pdev->id);
	if(config_i2s() < 0){
		i2s_dbg("resume to config to i2s  err!\n ");
		return -1;
	}
	return 0;
}
...
static struct platform_driver i2s_driver = {
	...
	//HTC_AUD_START, fei_tong modify for i2s suspend/resume
	//.suspend = i2s_drv_suspend,
	//.resume = i2s_drv_resume,
	//HTC_AUD_END
	.driver = {
		   .name = "i2s",
		   .owner = THIS_MODULE,
