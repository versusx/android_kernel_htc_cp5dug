...
static void insert_detect_work_func(struct work_struct *work)
{
	...
	if (hs_mgr_notifier.key_int_enable)
		hs_mgr_notifier.key_int_enable(0);
	set_35mm_hw_state(1);/* de-bouncing time for MIC output Volt */
	msleep(250);
	if ((hi->driver_one_wire_exist)&&(board_mfg_modem_mode() == 0)) {
	...
	hi->one_wire_mode = 0;
/*Check one wire accessory for every plug event*/
	if (hi->driver_one_wire_exist && adc > 1250) {
