...
#ifdef CONFIG_CABLE_DETECT_ACCESSORY
#define Z4TD_GPIO_USB_ID_PIN	102

void config_z4td_usb_id_gpios(bool output)
{
	gpio_request(Z4TD_GPIO_USB_ID_PIN, "cable_detect");
	if (output) {
		gpio_direction_output(Z4TD_GPIO_USB_ID_PIN, 1);
		gpio_set_value(Z4TD_GPIO_USB_ID_PIN, 1);
		pr_info("[CABLE] %s %d output high\n",
			__func__, Z4TD_GPIO_USB_ID_PIN);
	} else {
		gpio_direction_input(Z4TD_GPIO_USB_ID_PIN);
		pr_info("[CABLE] %s %d input none pull\n",
			__func__, Z4TD_GPIO_USB_ID_PIN);
	}
}

#define Z4TD_GPIO_USB_HOST_ENABLE_PIN        222

void z4td_enable_host_mode(bool enable)
{
	gpio_request(Z4TD_GPIO_USB_HOST_ENABLE_PIN, "cable_detect");
	if (enable) {
		gpio_direction_output(Z4TD_GPIO_USB_HOST_ENABLE_PIN, 1);
		gpio_set_value(Z4TD_GPIO_USB_HOST_ENABLE_PIN, 1);
		pr_info("[CABLE] %s %d output high\n",
				__func__, Z4TD_GPIO_USB_HOST_ENABLE_PIN);
	} else {
		gpio_direction_output(Z4TD_GPIO_USB_HOST_ENABLE_PIN, 0);
		gpio_set_value(Z4TD_GPIO_USB_HOST_ENABLE_PIN, 0);
...
static int64_t z4td_get_usbid_adc(void)
{
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.usb_id_pin_gpio        = Z4TD_GPIO_USB_ID_PIN,
	.config_usb_id_gpios    = config_z4td_usb_id_gpios,
	.get_adc_cb             = z4td_get_usbid_adc,
	//.enable_host_mode	= z4td_enable_host_mode,
};
...
{
	int ver = z4td_get_board_revision();
	...
	if(  BOARD_EVM == ver){
		...
		return snprintf(buf, 200,
			__stringify(EV_KEY) ":" __stringify(KEY_BACK) ":109:871:120:103"
			":" __stringify(EV_KEY) ":" __stringify(KEY_HOME) ":383:871:120:103"
			"\n");
		}
	else{
		return snprintf(buf, 200,
...
static struct synaptics_i2c_rmi_platform_data syn_ts_3k_data[] = {
{
		...
		.power = touch_power,
		.config = {
			0x5A,	0x34,	0x30,	0x34,	0x00,	0x7F,	0x03,	0x1E,
			...
			0x4C,	0x6C,	0x74,	0x1E,	0x05,	0x00,	0x02,	0x4A,
			...
			0x80,	0x80,	0x80,	0x80,	0x80,	0x80,	0x80,	0x80,
			0x80,	0x80,	0x80,	0x80,	0x80,	0x80,	0x80,	0x80,
			0x80,	0x80,	0x80,	0x8D,	0x80,	0x00,	0x0F,	0x00,
...
#ifdef CONFIG_PERFLOCK_HTC
static unsigned z4td_perf_acpu_table[] = {
        FREQ_LOWEST * 1000, /* PERF_LOCK_LOWEST */
        FREQ_LOW * 1000, /* PERF_LOCK_LOW */
        FREQ_MEDIUM * 1000, /* PERF_LOCK_MEDIUM */
        FREQ_HIGH * 1000, /* PERF_LOCK_HIGH */
        FREQ_HIGH * 1000, /* PERF_LOCK_HIGHEST */
};
static struct perflock_platform_data z4td_perflock_data = {
	.perf_acpu_table = z4td_perf_acpu_table,
	.table_size = ARRAY_SIZE(z4td_perf_acpu_table),
};

/* HTC_KER_ADD, Start, Winster @Perflock & Ceiling. */
static struct perflock_platform_data z4td_cpufreq_ceiling_data = {
	.perf_acpu_table = z4td_perf_acpu_table,
	.table_size = ARRAY_SIZE(z4td_perf_acpu_table),
};
...
static void __init z4td_init_machine(void)
{
...
#endif
#ifdef CONFIG_PERFLOCK_HTC
	perflock_init(&z4td_perflock_data);
	cpufreq_ceiling_init(&z4td_cpufreq_ceiling_data);
#endif
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_3K
	syn_init_vkeys();
#endif
...
static void __init z4td_init_early(void)
{
...
static void __init z4td_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
	...
	if (htc_get_config(HTC_DBG_FLAG_CP1_TD) & CP1_FLAG_MEM_LAYOUT_SM) {
		...
		printk(KERN_INFO "Memory layout with SM log\n");
	}
	else {
		mi->bank[1].start = LINUX_BASE_ADDR2;
		mi->bank[1].size  = LINUX_SIZE_ADDR2;
		printk(KERN_INFO "Memory layout without SM log\n");
	}

	printk(KERN_INFO "Setup %d memoy banks done \n", mi->nr_banks);
}

MACHINE_START(Z4TD, "UNKNOWN")
	.reserve	= sci_reserve,
	.map_io		= sci_map_io,
	.fixup		= z4td_fixup,
	.init_early	= z4td_init_early,
	.handle_irq	= gic_handle_irq,
	.init_irq	= sci_init_irq,
	.timer		= &__timer,
	.init_machine	= z4td_init_machine,
MACHINE_END
