...
static int perflock_released(struct inode *inode, struct file *file)
{
	mutex_lock(&lock);
	num--;
...
static int __init set_perflock_init(void){
	mutex_init(&lock);
	perf_lock_init(&media_perf_lock, TYPE_PERF_LOCK, PERF_LOCK_HIGHEST, "media");
