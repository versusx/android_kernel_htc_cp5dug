...
struct ktermios hs_wire_termios={
	.c_iflag=0,
	.c_oflag=0,
	.c_cflag=3263,
	.c_lflag=0,
...
static void setup_hs_tty(struct file *tty_fp)
	{
/*HTC_PER_START py_chang STE tty setting is different from QCOM platform*/
#if 0
		...
		old_fs = get_fs();
		set_fs(KERNEL_DS);
		tty_ioctl(tty_fp, TCSETS, (unsigned long)&hs_wire_termios);

		set_fs(old_fs);
/*HTC_PER_END py_chang STE tty setting is different from QCOM platform*/
	}
...
static int hs_1wire_init(void)
{
	...
	if (board_mfg_modem_mode() != 0){
		HS_LOG("radiorouter is enabled, one wire chip detection would be disabled\n");
		return -1;
	}
...
{
	gpio_set_value(hi->pdata.tx_level_shift_en, enable);
}
