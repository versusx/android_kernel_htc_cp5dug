...
/* HTC_KER_START icey, implement on mode charging animation */
	...
	list_for_each_entry(pos, &early_suspend_handlers, link) {
		if (pos->suspend != NULL) {
			if (debug_mask & DEBUG_VERBOSE)
				pr_info("early_suspend: enter %pf\n", pos->suspend);
			pos->suspend(pos);
			if (debug_mask & DEBUG_VERBOSE)
				pr_info("early_suspend: leave %pf \n", pos->suspend);
		}
	}
	mutex_unlock(&early_suspend_lock);
...
static void late_resume(struct work_struct *work)
{
	...
	list_for_each_entry_reverse(pos, &early_suspend_handlers, link) {
		if (pos->resume != NULL) {
			if (debug_mask & DEBUG_VERBOSE)
				pr_info("late_resume: enter %pf\n", pos->resume);
			pos->resume(pos);
			if (debug_mask & DEBUG_VERBOSE)
				pr_info("late_resume: leave %pf\n", pos->resume);
		}
	}
	if (debug_mask & DEBUG_SUSPEND)
...
extern bool dpm_enable_debug;

//void __weak pm_enable_debug(void) {}
...
static int __init __suspend_flag(char *str)
{
	...
	if (flag & 0x02)
		debug_mask |= DEBUG_SUSPEND | DEBUG_VERBOSE;
	if (flag & 0x04)
		dpm_enable_debug = true;
	if (flag & 0x08)
		pm_enable_debug();
	return 0;
}*/
/* Disable the power debug flag
early_param("debugflag", __suspend_flag);
*/
