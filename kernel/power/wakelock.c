...
/* HTC_KER_START Michael_Xi : Move sys_sync calls to a separate workqueue */
...
/* HTC_KER_END Michael_Xi : Move sys_sync calls to a separate workqueue */
...
void suspend_sys_sync_queue(void)
{
	...
	pr_info("Queued sys_sync work queue\n");
	spin_lock(&suspend_sys_sync_lock);
 ...
 */
#define SUSPEND_SYS_SYNC_TIMEOUT (HZ/32)
...
static void suspend(struct work_struct *work)
{
	...
	suspend_sys_sync_queue(); /* HTC_KER_MOD Michael_Xi : Move sys_sync calls to a separate workqueue */
	if (debug_mask & DEBUG_SUSPEND)
...
err_suspend_sys_sync_work_queue: /* HTC_KER_ADD Michael_Xi : Move sys_sync calls to a separate workqueue */
...
static void  __exit wakelocks_exit(void)
{
	...
	destroy_workqueue(suspend_sys_sync_work_queue); /* HTC_KER_ADD Michael_Xi : Move sys_sync calls to a separate workqueue */
	platform_driver_unregister(&power_driver);
	platform_device_unregister(&power_device);
	wake_lock_destroy(&suspend_backoff_lock);
	wake_lock_destroy(&unknown_wakeup);
